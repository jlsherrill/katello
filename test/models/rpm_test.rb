...
module Katello
  ...
  class RpmSortTest < ActiveSupport::TestCase
    ...
    def test_min_version_filter
      results = Rpm.search_version_range("1.0.0")
      assert_equal ["abc123-4", "abc123-6"], results.map(&:uuid).sort

      results = Rpm.search_version_range("1")
      expected = @all_ids - ["abc123-8"]
      assert_equal expected, results.map(&:uuid).sort

      results = Rpm.search_version_range("1.0.0-1.0")
      expected = ["abc123-4", "abc123-6"]
      assert_equal expected, results.map(&:uuid).sort

      results = Rpm.search_version_range("1.0.0-1el4")
      expected = ["abc123-1", "abc123-2", "abc123-4", "abc123-5", "abc123-6"]
      assert_equal expected, results.map(&:uuid).sort
    end

    def test_max_version_filter
      results = Rpm.search_version_range(nil, "1:1.0.0")
      assert_equal @all_ids - ["abc123-2"], results.map(&:uuid).sort

      results = Rpm.search_version_range(nil, "0:1.0.0")
      assert_equal ["abc123-8"], results.map(&:uuid).sort
    end

    def test_version_range_filter
      results = Rpm.search_version_range("0.9.1", "2:0.9.1")
      assert_equal @all_ids, results.map(&:uuid).sort

      results = Rpm.search_version_range("1.0.0", "1.0.0-0.9.1")
      assert_empty results

      results = Rpm.search_version_range("1.0.0-1", "1.0.0-1.2")
      expected = ["abc123-1", "abc123-2", "abc123-5"]
      assert_equal expected, results.map(&:uuid).sort
    end

    def test_equal_filter
      results = Rpm.search_version_equal("1.0.0")
      expected = @all_ids - ["abc123-4", "abc123-6", "abc123-8"]
      assert_equal expected, results.map(&:uuid).sort

      results = Rpm.search_version_equal("1:1.0.0")
      expected = ["abc123-2"]
      assert_equal expected, results.map(&:uuid).sort

      results = Rpm.search_version_equal("1:1.0.0-1.0")
      expected = ["abc123-2"]
      assert_equal expected, results.map(&:uuid).sort
    end
  end
