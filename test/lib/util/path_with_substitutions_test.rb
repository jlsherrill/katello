  ...
  module Util
    class PathWithSubstitutionsTest < ActiveSupport::TestCase
      ...
      def test_resolve_substitutions_releasever
        ...
        cdn_resource = mock
        cdn_resource.expects(:fetch_substitutions).with('/content/dist/rhel/server/5/').returns(@releasever_list)

        resolved = path.resolve_substitutions(cdn_resource)
      ...
      def test_resolve_substitutions_arch
        ...
        cdn_resource = mock
        cdn_resource.expects(:fetch_substitutions).with('/content/dist/rhel/server/5/5Server/').returns(@arch_list)
