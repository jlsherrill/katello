module Katello
  class Repository < Katello::Model
    ...
    def packages_without_errata
      Rpm.in_repositories(self).where("#{Rpm.table_name}.filename NOT in (?)", errata_filenames.to_sql)
    end
    ...
    def errata_filenames
      Katello::ErratumPackage.joins(:erratum => :repository_errata).
          where("#{RepositoryErratum.table_name}.repository_id" => self.id).select(:filename)
    end
