module Katello
  class Api::V2::RepositoriesController < Api::V2::ApiController
    ...
    param :rpm_id, String, :desc => N_("UUID of a package to find repositories that contain the rpm")
    ...
    def index_relation
      ...
      query = query.joins(:rpms).where("#{Rpm.table_name}.uuid" => params[:rpm_id]) if params[:rpm_id]
    ...
    def remove_content
      fail _("No content ids provided") if (params[:uuids] || params[:ids]).blank?
